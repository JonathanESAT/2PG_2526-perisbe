...
void MaterialBasic::init() {
  ...
  //2: Load the source code to the requested shaders.
	v_shader->loadSource(EDK::dev::Shader::Type::kType_Vertex,
						kVertexShader, strlen(kVertexShader));

	f_shader->loadSource(EDK::dev::Shader::Type::kType_Fragment,
						kFragmentShader, strlen(kFragmentShader));
