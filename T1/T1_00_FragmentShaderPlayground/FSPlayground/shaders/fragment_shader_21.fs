...
void main() {
  ...
  //Blur:
  int blur_offset = 3;
  int min_x = int(clamp(gl_FragCoord.x - blur_offset, 0.0, u_window_size.x));
  int min_y = int(clamp(gl_FragCoord.y - blur_offset, 0.0, u_window_size.y));
  int max_x = int(clamp(gl_FragCoord.x + blur_offset, 0.0, u_window_size.x));
  int max_y = int(clamp(gl_FragCoord.y + blur_offset, 0.0, u_window_size.y));
  vec3 color = vec3(0.0,0.0,0.0);

  for(int i = min_y; i < max_y; ++i){
    for(int j = min_x; j < max_x; ++j){
      vec2 fakeCoords = vec2(gl_FragCoord.x + j, gl_FragCoord.y + i); 
      ...
      //Pfffiuuuu:
      float wave_amplitude = 20.0 * r;
      float pfiu_multiplier = 100.0 * r;
      float offset_pfiu = sin(fakeCoords.x / u_window_size.x * 6.28 * pfiu_multiplier + u_time * 0.01) * wave_amplitude;
      ...
      //Final color:
      color += mix(bck_color, pacman_color, es * bs * ms);

      
    }
  }

  color /= blur_offset * blur_offset;
