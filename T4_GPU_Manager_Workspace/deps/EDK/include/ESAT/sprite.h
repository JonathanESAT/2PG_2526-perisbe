...
namespace esat {
  ...
  SpriteHandle SpriteFromMemory(int width, int height, 
                                const unsigned char *data_RGBA = 0L);
  void SpriteUpdateFromMemory(SpriteHandle, 
                              const unsigned char *data_RGBA);

  SpriteHandle SubSprite(SpriteHandle origin, int x, int y, 
                         int width, int height);

  const unsigned int SpriteID(SpriteHandle img);
  void SpriteRelease(SpriteHandle img);

  const int SpriteHeight(SpriteHandle img);
  const int SpriteWidth(SpriteHandle img);
  void SpriteGetPixel(SpriteHandle img, int x, int y, 
                      unsigned char outRGBA[4]);

  //Interpolation is an EXPERIMENTAL FEATURE, it may not work in 
  //all cases.
  ...
  struct SpriteTransform {
    float x,y;                //Default 0, 0. 
    float angle;              //In radians (default 0).
    float scale_x, scale_y;   //Ddefault 1, 1.
    float sprite_origin_x,
          sprite_origin_y;    //Origin relative to sprite 
